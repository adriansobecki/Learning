...
QVariant TableModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        QVariantList data = database->executeQuery(QString("SELECT %1 FROM (SELECT id, first_name, last_name, email, gender, row_number()").arg(database->getColumnName(index.column())) +
